...
class movie_dataset_etl:
    ...
    def transform(self):
        ...
        ratings_cleaned = self.ratings.copy()
    ...
    def _transform_credits(self):
        ...
        cast = self._normalize_data(df, "cast", "id", "m")
        crew = self._normalize_data(df, "crew", "id", "m")
        
        # Data cleaning process for cast and crew
        cast.drop_duplicates(subset=["cast_id", "credit_id"], keep="first", inplace=True)
        crew.drop_duplicates(subset=["id", "credit_id", "m_id", "name"], keep="first", inplace=True)
    ...
    def _transform_keywords(self):
        ...
        df = self._normalize_data(df, "keywords", "id", "m")
    ...
    def _transform_movies_metadata(self):
        ...
        # Drop duplicates
        df.drop_duplicates(subset=["id", "title", "imdb_id"], keep="first", inplace=True)
        
        # Drop unnecessary columns & extract belongs_to_collection data
        belongs_to_collection = df[["belongs_to_collection", "id"]].copy()
        ...
        # Extract genres data
        genres = self._normalize_data(df, "genres", "id", "m")
        df.drop(columns=["genres"], inplace=True)
        
        # Extract production_companies data
        production_companies = self._normalize_data(df, "production_companies", "id", "m")
        df.drop(columns=["production_companies"], inplace=True)
        
        # Extract production_countries data
        production_countries = self._normalize_data(df, "production_countries", "id", "m")
        df.drop(columns=["production_countries"], inplace=True)
        
        # Extract spoken_languages data
        spoken_languages = self._normalize_data(df, "spoken_languages", "id", "m")
    ...
    def _extract_dict_values(
        ...
        parsed = df[column_name].apply(self._safe_parse_collection)
        ...
        return pd.concat([df, normalized], axis=1).drop(column_name, axis=1)